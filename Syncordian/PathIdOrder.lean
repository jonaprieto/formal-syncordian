...
theorem Segment.compare_def (a b : Segment) :
    compare a b = (match compare a.digit b.digit with
                   | .eq => compare a.peer b.peer
                   | result => result) := rfl

theorem Segment.compare_eq_eq {a b : Segment} : compare a b = .eq ↔ a = b := by
...
theorem Segment.lt_iff {a b : Segment} :
    Segment.lt a b ↔ a.digit < b.digit ∨ (a.digit = b.digit ∧ a.peer < b.peer) := by
  unfold Segment.lt
  rw [Segment.compare_def]
...
theorem Segment.lt_trans {a b c : Segment}
    (hab : Segment.lt a b) (hbc : Segment.lt b c) : Segment.lt a c := by
  rw [Segment.lt_iff] at hab hbc ⊢
  cases hab with
  | inl hd =>
    cases hbc with
    | inl hd' => exact Or.inl (Nat.lt_trans hd hd')
    | inr hd' => exact Or.inl (hd'.1 ▸ hd)
  | inr hp =>
    cases hbc with
    | inl hd' => exact Or.inl (hp.1 ▸ hd')
    | inr hp' => exact Or.inr ⟨hp.1.trans hp'.1, Nat.lt_trans hp.2 hp'.2⟩

theorem Segment.lt_total (a b : Segment) :
    Segment.lt a b ∨ a = b ∨ Segment.lt b a := by
  simp only [Segment.lt_iff]
  rcases Nat.lt_trichotomy a.digit b.digit with h | h | h
  · exact Or.inl (Or.inl h)
  · rcases Nat.lt_trichotomy a.peer b.peer with hp | hp | hp
...
theorem Lex.append_right : ∀ (as : List Segment) (x : Segment) (xs : List Segment),
    Lex as (as ++ x :: xs) := by
  intro as x xs
  induction as with
...
theorem Lex.append_of_not_prefix : ∀ {as bs : List Segment},
...
theorem Lex.prefix_below : ∀ {as bs : List Segment},
...
theorem lex_iff_compareList : ∀ (as bs : List Segment),
  ...
  | cons a as ih =>
    ...
    | cons b bs =>
      ...
      | lt =>
        exact ⟨fun _ => rfl, fun _ => Lex.head hc as bs⟩
      | eq =>
        ...
        · intro h
          cases h with
          | head h' _ _ => exact absurd (hc.symm.trans h') (by simp)
      ...
      | gt =>
        ...
        · intro h
          cases h with
          | head h' _ _ => rw [h'] at hc; cases hc
          | tail _ _ => rw [Segment.compare_eq_eq.mpr rfl] at hc; cases hc
        · intro h; cases h

theorem belowL_lt : ∀ (s : Segment) (ts : List Segment),
  ...
  | nil =>
    ...
    exact Lex.head (a := { s with peer := s.peer - 1 }) (b := s)
      (Segment.lt_iff.mpr (Or.inr ⟨rfl, Nat.sub_lt h' Nat.one_pos⟩)) _ _
  | cons t ts ih =>
    intro h
    exact Lex.tail _ (ih t h)

theorem Path.below_lt (p : Path) (hp : p.WellFormed) :
...
theorem PathId.lt_path {p q : Path} :
...
theorem PathId.infimum_lt {x : PathId} (h : x ≠ .infimum) : PathId.lt .infimum x := by
  cases x with
  | infimum => exact absurd rfl h
  | path _ => rfl
  | supremum => rfl

theorem PathId.lt_supremum {x : PathId} (h : x ≠ .supremum) : PathId.lt x .supremum := by
  cases x with
...
theorem PathId.lt_trans {a b c : PathId}
    (hab : PathId.lt a b) (hbc : PathId.lt b c) : PathId.lt a c := by
...
theorem PathId.lt_total (a b : PathId) (h : a ≠ b) :
...
instance instDecidableLtWellFormed (a b : { x : PathId // x.WellFormed }) : Decidable (a < b) :=
